...
from library.classifier.classifier_ordering_loader import ClassifierRuleDictionary
...
"""
This vector file is a refined version of tree_vectors.py 
which include the narrative scheme features:
C_MORPHY_COMMON = 'morphy_commonness'
C_MORPHY_SCALE = 'morphy_scale'
C_SYNSET_COMMON = 'synset_commonness'
C_SYNSET_SCALE = 'synset_scale'
"""
...
crd = ClassifierRuleDictionary.get_single_instance()
...
class EEVector(CombinedVector):
    ...
    def __init__(self, event_vector1, event_vector2, flag = None):
        ...
        string_lemma_1 = self.v1.get_value('string')
        string_lemma_2 = self.v2.get_value('string')
        
        
        synsets_event_1 = None
        if len(wn.synsets(string_lemma_1, 'v')) > 0:
            synsets_event_1 = wn.synsets(string_lemma_1, 'v')[0].lemma_names
        verb_morphy_1 = wn.morphy(string_lemma_1, 'v')
        logger.info(synsets_event_1)
        logger.info(verb_morphy_1)
        
        synsets_event_2 = None
        if len(wn.synsets(string_lemma_2, 'v')) > 0:
            synsets_event_2 = wn.synsets(string_lemma_2, 'v')[0].lemma_names
        verb_morphy_2 = wn.morphy(string_lemma_2, 'v')
        logger.info(synsets_event_2)
        logger.info(verb_morphy_2)
        
        morphy_value = crd.check_in_dict(verb_morphy_1, verb_morphy_2)
        logger.info("morphy_value")
        logger.info(morphy_value)
        
        synset_value = numpy.zeros(2)
        if synsets_event_1 != None and synsets_event_2 != None:
            for l_1, l_2 in itertools.product(synsets_event_1, synsets_event_2):
                logger.info("%s %s" %(l_1, l_2))
                logger.info(crd.check_in_dict(l_1, l_2))
                synset_value += crd.check_in_dict(l_1, l_2)
        """
        commonness = log2( sum(two_values) + 2)
        scale = log2( (value1 + 1)/(value2 + 2) )
        """
        self.morphy_commonness = numpy.log2( morphy_value[0] + morphy_value[1] + 2 )
        self.morphy_scale = numpy.log2( float(morphy_value[0] + 1)/( morphy_value[1] + 1) )
        self.synset_commonness = numpy.log2( synset_value[0] + synset_value[1] + 2 )
        self.synset_scale = numpy.log2( float(synset_value[0] + 1)/( synset_value[1] + 1) )
        logger.info(self.morphy_commonness)
        logger.info(self.morphy_scale)
        logger.info(self.synset_commonness)
        logger.info(self.synset_scale)
        
        logger.info("======================END DEBUG===========================")
        if self.morphy_commonness != 1:
            self._mutual_features.append( (C_MORPHY_COMMON, float(self.morphy_commonness)) )
            self._mutual_features.append( (C_MORPHY_SCALE, float(self.morphy_scale)))
        if self.synset_commonness != 1:
            self._mutual_features.append((C_SYNSET_COMMON, float(self.synset_commonness)))
            self._mutual_features.append( (C_SYNSET_SCALE, float(self.synset_scale)))
