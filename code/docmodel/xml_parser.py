...
from library.timeMLspec import INSTANCE, TIMEX, TID, EIID
import logging
...
class XmlDocument:
    ...
    # Tuan added this on October 23rd
    def collect_tid_and_eiid(self):
        """
        Because TLINK relates to only TIMEX3 and EVENT tags,
        collecting them in the right order help retrieve them
        easier.
        """
        self._tid_and_eiid = []
        for element in self.elements:
            if element.tag == INSTANCE and element.is_opening_tag():
                self._tid_and_eiid.append( element.attrs[EIID] )
            if element.tag == TIMEX and element.is_opening_tag():
                self._tid_and_eiid.append( element.attrs[TID] )
        return self._tid_and_eiid
    ...
    # TUAN added October 21
    def _add_doc_element_with_check_old(self, current_element):
        """Append an XmlDocElement to the elements variable. Sets the previous field of
        the added element and the next field of the previous element is there was
        one. Also increments the last_index variable.
        One more thing is keeping track of the last opening tag, if 
        """
        
        if current_element.is_closing_tag() and current_element.tag == 'TEXT':
            logger.warn('Ahha')
            logger.warn(self.opening_tag_stack[-1])
             
        if current_element.is_closing_tag() and current_element.tag == 'TIMEX3':
            print (self.opening_tag_stack)
                    
        if current_element.is_closing_tag():
            if current_element.tag != self.opening_tag_stack[-1]:
                return False
            else:
                self.opening_tag_stack.pop()
        previous_element = None
        last_index = self._get_last_index()
        if current_element.is_opening_tag():
            self.opening_tag_stack.append(current_element.tag)
        if last_index > -1:
            previous_element = self.elements[last_index]
            previous_element.set_next(current_element)
        current_element.set_previous(previous_element)
    ...
    # TUAN added November 6
    def _add_doc_element_with_check(self, current_element):
        """Append an XmlDocElement to the elements variable. Sets the previous field of
        the added element and the next field of the previous element is there was
        one. Also increments the last_index variable.
        One more thing is keeping track of the last opening tag, if 
        ...
        """
        if current_element.is_opening_tag():
            if current_element.tag in self.opening_tag_stack:
                return False
        if current_element.is_closing_tag():
            if current_element.tag != self.opening_tag_stack[-1]:
                return False
    ...
    def add_tlink(self, reltype, id1, id2, origin):
        ...
        closing_tag.insert_element_before(XmlDocElement ('\n', tag = None))
        self.add_to_tags_dictionary('TLINK', new_opening_element)
    
    """
    Tuan added on November 7
    """
    ...
    def save_to_file(self, filename):
        ...
        while element:
            ...
            if element.is_text_element() and not string.find('xml version="1.0"') != -1:
                string = escape(string)
            elif element.is_opening_tag() and element.tag == 'lex':
                #TODO: as_xml_tag has some problem here
    ...
    ### TUAN added in October
    def set_dct_timex(self, dct_time):
        self._dct_time = dct_time
        
    def _generate_all_sentences (self):
        """
        This function would generate all XmlSentence in the xml document
        and store in a data structure. XmlDocElement inside a sentence 
        should also be binded to a sentence index.
        That could help to find features in the same sentence later. 
        For example:
            - tree-parsed features
            - tokens between two events
        """
...
#             self._dct_time = None
            ...
            lex_counter = 0
            for el in self:
                if el.is_opening_tag() and el.tag == 's':
                    current_sent = XmlSentence(sent_counter)
                    lex_counter = 0
                if el.is_closing_tag() and el.tag == 's':
                    self._sentences.append(current_sent)
                    current_sent = None
                    sent_counter += 1
                if el.is_opening_tag() and el.tag == 'lex':
                    current_sent.add_lex(el)
                    lex_counter += 1
                if el.tag == 'EVENT':
                    # Assume that event only extend on one token
                    if el.is_opening_tag():
                        extend = (lex_counter, lex_counter)
                        if current_sent != None:    
                            current_sent.add_event(el, extend)
                if el.tag == 'TIMEX3':
                    if el.is_opening_tag():
                        # While currently EVENT is inside a lex, TIMEX3 is outside lex
                        extend = lex_counter + 1
                        opening_timex = el
#                         if self._dct_time == None:
#                             self._dct_time = el
                    if el.is_closing_tag():
                        extend = (extend, lex_counter)
                        if current_sent != None:  
                            current_sent.add_time(opening_timex, extend)
    
    def get_sentences(self):
        self._generate_all_sentences()
        return self._sentences
    
    def get_dct (self):
        return self._dct_time
    
### TUAN added in October
class XmlSentence:
    """
    An XmlSentence is basically a data structure that should be stored inside an XmlDocument,
    and it store pointer to including XmlDocElement inside that XmlDocument. It is added later
    to help alleviating the work on sentences in the documents.
    It should support loading some parsing structure (later), and support operations on parsed
    structure.
    
    Instance variables
        _index - the position in the xmlDocument
        _lexs - list of pointers to XmlDocElements in the XmlDocument that is lex
                but if one want to get the token lemma, they still have to use lex.next()
        _length - number of tokens in the sentences
        _events - list of pointers point to XmlDocElements that are EVENT tags,
                        and extends of the event in the sentence.
        _timexs - list of pointers point to XmlDocElements that are TIMEX3 tags,
                        and extends of the event in the sentence.
    """
    EVENT_FLAG = 'event'
    TIMEX_FLAG = 'timex'
    def __init__(self, index, lexs = []):
        self._index = index
        self._lexs = lexs
        self._length = len(self._lexs)
        self._events = []
        self._timexs = []
        self._ets = []
    
    def add_lex(self, lex):
        self._lexs.append( lex )
        self._length += 1
    
    def add_event (self, event, extend):
        """
        Add an event with extend to be the first and last lex index
        """
        self._events.append( (event, extend) )
        self._ets.append( (event, extend, XmlSentence.EVENT_FLAG) )
    
    def add_time (self, timex, extend):
        """
        Add a timex with extend to be the first and last lex index
        """
        self._timexs.append( (timex, extend) )
        self._ets.append( (timex, extend, XmlSentence.TIMEX_FLAG) )
    ...
    def get_times_and_events(self):
        return self._ets
    
    def get_tokens(self, extend, filter = lambda lex:True, include = True):
        """
        Get all lex tokens inside the sentence given indices of
        the first and last token.
        Parameters:
            extend - a tuple of begin_index, last_index
            filter - a function to select which token to return
            include - whether the filter to filter tokens to return or to remove
        """
        result_tokens = []
        for lex in self._lexs[extend[0]:extend[1] + 1]:
            if bool(filter(lex)) == bool(include):
                lex_text = lex
                while not lex_text.is_text_element():
                    lex_text = lex_text.get_next()
                result_tokens.append(lex_text.content)
    ...
    def as_xml_tag(self):
        ...
        if self.attrs:
            for (a,v) in self.attrs.items():
                ...
                a_list.append((a, v))
        if a_list:
            # Tuan Do: I change the value with adding of ""
