...
WNHOME = environ.get('WNHOME', {
    'mac': ":",
    'dos': "C:\wn16",
    'nt': "C:\wn16"}.get(os.name, "/usr/local/wordnet1.6"))

#modification to work in the evita environment
WNSEARCHDIR = os.path.abspath(os.path.dirname(sys.argv[0]))+'/Dicts/'
...
def setupLexnames():
    lex_file = open(WNSEARCHDIR+'/lexnames')
    for l in lex_file.readlines():
...
#We're only using wordnet for nouns in evita
